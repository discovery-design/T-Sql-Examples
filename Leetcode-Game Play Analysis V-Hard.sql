...
ROUND(Cast(count(B.player_id) as decimal)/Cast(count(A.player_id) as decimal),2) Day1_retention
From (Select player_id,MIN(event_date) as install_dt From Activity
Group By player_id
) as A
Left Join Activity B On
A.player_id=B.player_id And DATEADD(day,1,A.install_dt)=B.Event_date
